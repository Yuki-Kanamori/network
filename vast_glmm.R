...
#####Data#####
setwd(dir = dirname)
load("isi2_glmm.RData")
glmm = isi2_glmm$arithmean
summary(glmm)
DG = read.csv("Data_Geostat_na.csv")
summary(DG)
sakana = c("isigarei", "konosiro", "kouika", "kurumaebi", "maanago", "makogarei", "suzuki")[4]
DG = filter(DG, Sp == sakana)
DG2 = ddply(DG, .(knot_i), summarize, Lon = mean(Lon), Lat = mean(Lat))
glmm = merge(glmm, DG2, by = "knot_i")

ef = ddply(DG, .(knot_i, Year, Month), summarize, m_Effort = mean(Effort))
glmm = merge(glmm, ef, by = c("knot_i", "Year", "Month"))

df = glmm

# glmmでデータを補完して、足切りしたもの
for(i in 1:5){
  dirname = "C:/kana/Dropbox/Network/revised_data"
  setwd(dir = dirname)
  glmm = read.csv("tok_glmm1.csv")
  summary(glmm)
  sakana = c("isigarei", "konosiro", "kouika", "kurumaebi", "maanago", "makogarei", "suzuki")[4]
  glmm = filter(glmm, Sp == sakana)
  summary(glmm)
  ...
  # adde season
  df = rbind(filter(df, between(Month, 1, 3)) %>% mutate(Season = "WS"), 
             filter(df, between(Month, 4, 6)) %>% mutate(Season = "SS"), 
             filter(df, between(Month, 7, 9)) %>% mutate(Season = "SA"),
             filter(df, between(Month, 10, 12)) %>% mutate(Season = "AW"))
  # test = data.frame(unique(Data_Geostat$Season))
  # test2 = as.numeric(as.factor(test[, 1])) -1
  ...
  head(df)
  # # df <- df[, c("Year", "Month", "Lat", "Lon", "geomean_dens", "m_Effort")]
  # # colnames(df) <- c("Year", "Month", "Lat", "Lon", "Catch_KG", "Effort")
  # df = df[, c("Year", "Month", "m_lat", "m_lon", "mean_dens", "m_Effort")]
  # colnames(df) = c("Year", "Month", "Lat", "Lon", "Catch_KG", "Effort")
  
  #if season
  df = df[, c("Year", "Season", "m_lat", "m_lon", "mean_dens", "m_Effort")]
  colnames(df) = c("Year", "Season", "Lat", "Lon", "Catch_KG", "Effort")
