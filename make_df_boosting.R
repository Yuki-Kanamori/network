...
do = data %>% select(species, year, season, n_season, knot_i, log_abundance, layer, env, w_mean) %>% 
  filter(env == "DO") %>% spread(key = layer, value = w_mean) 
colnames(do)[8:12] = c("do_0","do_5","do_10","do_20","do_50")
sal = data %>% select(species, year, season, n_season, knot_i, log_abundance, layer, env, w_mean) %>% 
  filter(env == "salinity") %>% spread(key = layer, value = w_mean) 
colnames(sal)[8:12] = c("sal_0","sal_5","sal_10","sal_20","sal_50")
wt = data %>% select(species, year, season, n_season, knot_i, log_abundance, layer, env, w_mean) %>% 
  filter(env == "w_temperature") %>% spread(key = layer, value = w_mean)
colnames(wt)[8:12] = c("wt_0","wt_5","wt_10","wt_20","wt_50")
head(do)
isigarei = left_join(do, sal, by = c("species", "year", "season", "n_season", "knot_i", "log_abundance"))
isigarei = left_join(isigarei, wt, by = c("species", "year", "season", "n_season", "knot_i", "log_abundance"))
...
colnames(wt)[8:12] = c("wt_0","wt_5","wt_10","wt_20","wt_50")
head(do)
konosiro = left_join(do, sal, by = c("species", "year", "season", "n_season","knot_i", "log_abundance"))
konosiro = left_join(konosiro, wt, by = c("species", "year", "season","n_season","knot_i", "log_abundance"))
...
colnames(wt)[8:12] = c("wt_0","wt_5","wt_10","wt_20","wt_50")
head(do)
kouika = left_join(do, sal, by = c("species", "year", "season", "n_season","knot_i", "log_abundance"))
kouika = left_join(kouika, wt, by = c("species", "year", "season","n_season", "knot_i", "log_abundance"))
...
colnames(wt)[8:12] = c("wt_0","wt_5","wt_10","wt_20","wt_50")
head(do)
kurumaebi = left_join(do, sal, by = c("species", "year", "season","n_season","knot_i", "log_abundance"))
kurumaebi = left_join(kurumaebi, wt, by = c("species", "year", "season","n_season", "knot_i", "log_abundance"))
...
colnames(wt)[8:12] = c("wt_0","wt_5","wt_10","wt_20","wt_50")
head(do)
maanago = left_join(do, sal, by = c("species", "year", "season","n_season","knot_i", "log_abundance"))
maanago = left_join(maanago, wt, by = c("species", "year", "season","n_season", "knot_i", "log_abundance"))
...
colnames(wt)[8:12] = c("wt_0","wt_5","wt_10","wt_20","wt_50")
head(do)
makogarei = left_join(do, sal, by = c("species", "year", "season","n_season", "knot_i", "log_abundance"))
makogarei = left_join(makogarei, wt, by = c("species", "year", "season","n_season", "knot_i", "log_abundance"))
...
colnames(wt)[8:12] = c("wt_0","wt_5","wt_10","wt_20","wt_50")
head(do)
suzuki = left_join(do, sal, by = c("species", "year", "season","n_season","knot_i", "log_abundance"))
suzuki = left_join(suzuki, wt, by = c("species", "year", "season","n_season","knot_i", "log_abundance"))
