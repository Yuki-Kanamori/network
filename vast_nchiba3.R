...
tok = rbind(filter(tok, between(M, 1, 3)) %>% mutate(Season = "WS"), 
            filter(tok, between(M, 4, 6)) %>% mutate(Season = "SS"), 
            filter(tok, between(M, 7, 9)) %>% mutate(Season = "SA"),
            filter(tok, between(M, 10, 12)) %>% mutate(Season = "AW"))
